function [m,n,myTri1,myTri2,myTri3]=triangleProjection(myCylinder, vertex, face)
	...
	myTri1=zeros(face_x,3);
	myTri2=zeros(face_x,3);
	myTri3=zeros(face_x,3);

	[m,n]=size(X);

	[TH,R,Z] = cart2pol(tri1(:,1),tri1(:,2),tri1(:, 3));
	TH(TH<0)=TH(TH<0)+2*pi;
	myTri1(:,1)=TH/(2*pi)*(n-1);
	myTri1(:,2)=(Z-min(Z))/(max(Z)-min(Z))*(m-1);
    myTri1(:,3)=R;

	[TH,R,Z] = cart2pol(tri2(:,1),tri2(:,2),tri2(:,3));
	TH(TH<0)=TH(TH<0)+2*pi;
	myTri2(:,1)=TH/(2*pi)*(n-1);
	myTri2(:,2)=(Z-min(Z))/(max(Z)-min(Z))*(m-1);
    myTri2(:,3)=R;

	[TH,R,Z] = cart2pol(tri3(:,1),tri3(:,2),tri3(:,3));
	TH(TH<0)=TH(TH<0)+2*pi;
	myTri3(:,1)=TH/(2*pi)*(n-1);
	myTri3(:,2)=(Z-min(Z))/(max(Z)-min(Z))*(m-1);
    myTri3(:,3)=R;
end
